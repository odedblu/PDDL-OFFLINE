    ...
    class SDRPlanner
    {
        ...
        public void OfflinePlanning(string sPath, Domain domain, Problem problem)
        {
            ...
            while (!bDone)
            {
                ...
                lPlan = null;
                while (lPlan == null && pssCurrent != null)
                {
                    Debug.Write(c + ") Checking state " + pssCurrent.ToString() + " |P|=" + cPlanning + " |A|=" + cActions + " T=" + ((long)(DateTime.Now-dtStart).TotalSeconds)
                ...
                if (!bDone && lPlan != null)
                {
                    ...
                    foreach (string sAction in lPlan)
                    {
                        if (!IsReasoningAction(sAction.ToLower()))
                        {
                            ...
                            if (false)//use all observation actions???
                            {
