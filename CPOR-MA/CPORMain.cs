    ...
    class CPORMain
    {
        ...
        */
        ...
        class TestBenchmarkThread
        {
            ...
            public StageTwoPlanPlusProblem Run()
            {
                if (UseCLG)
                {
                ...
                else
                {
                    ...
                    return TestBenchmark(BenchmarkPath, Benchmark, Trials, WriteResults);
                }
            }

            public static StageTwoPlanPlusProblem TestBenchmark(string sBenchmarkPath, string sBenchmark, int cTrials, bool bWriteResults)
            {
        ...
        static StageTwoPlanPlusProblem TestBenchmark(string sBenchmarkPath, string sBenchmark, int cTrials, bool bWriteResults, 
            bool bSeparateThread)
        {
            TestBenchmarkThread tbt = new TestBenchmarkThread(sBenchmarkPath, sBenchmark, cTrials, bWriteResults);
            StageTwoPlanPlusProblem stageTwoPlanPlusProblem = null;
            if (!bSeparateThread)
            {
                stageTwoPlanPlusProblem = tbt.Run();
                return stageTwoPlanPlusProblem;
            }
            else
            {
                Thread t = new Thread(() => { stageTwoPlanPlusProblem = tbt.Run();});
        ...
        static StageTwoPlanPlusProblem TestAll(string sBenchmarkPath, string[] asBenchmarks, int cTrials, bool bMultiThread)
        {
            int cMaxThreads = 3;
            ...
            **/
            ...
            {
                stageTwoPlanPlusProblem = TestBenchmark(sBenchmarkPath, asBenchmarks[i], cTrials, true, bMultiThread);
                i++;
            }
        ...
        // shashank - main call 
        public StageTwoPlanPlusProblem cpormain(String sBenchmarkPath, String[] asBenchmarks, int cFailures)
        {            
            /*
            foreach (string sBenchmark in asBenchmarks)
            {
            ...
            */
            ...
            // guy's approach: did not work
            // SDRPlanner sdr = new SDRPlanner(sPath, domain, problem);
            this.ccFailure = cFailures;
            // RandomGenerator.Init(cFailures * 1111);
            // StageTwoPlanPlusProblem stageTwoPlanPlusProblem = sdr.Start();

            StageTwoPlanPlusProblem stageTwoPlanPlusProblem = TestAll(sBenchmarkPath, asBenchmarks, 1, false);
            ...
            StreamWriter swTime = new StreamWriter(@"C:\Users\shekhar\Downloads\SDR\newV.txt", true);
