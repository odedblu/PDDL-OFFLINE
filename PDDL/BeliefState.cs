    ...
    public class BeliefState
    {
        ...
        */
        ...
        public State WriteTaggedDomainAndProblem(string sDomainFile, string sProblemFile, CompoundFormula cfGoal, List<Action> lAppliedActions, out int cTags, out MemoryStream msModels)
        {
            ...
            WriteTaggedDomainAndProblem(sDomainFile, sProblemFile, cfGoal, lStates, false, out cTags, out msModels);
            return lStates[0];
        }
        ...
        public void WriteTaggedDomainAndProblem(string sDomainFile, string sProblemFile, CompoundFormula cfGoal, List<State> lStates, bool bOnlyIdentifyStates, out int cTags, out MemoryStream msModels)
        {
        ...
        private List<List<Predicate>> ChooseDiverseStateSet(int cStates, List<Predicate> lCurrentTag)
        {
            ...
            int cChosenStates = 0;
            //while ((cChosenStates < (cStates - 1) && cFailedAttempts < cStates * 2) || lStates.Count == 0)//1 here if we want to add state negation
            ...
            if (false && lNotAppearing.Count > 0)
            {
