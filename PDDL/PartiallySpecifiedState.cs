...
namespace PDDL
{
    public class PartiallySpecifiedState
    {
        ...
        public Dictionary<GroundedPredicate, HashSet<GroundedPredicate>> m_dRelevantVariablesForPrecondition;
...
*/
        ...
        private bool m_bGoalState = false;
        public bool IsGoalState()
        {
            if (m_bGoalStateChecked)
                return m_bGoalState;
            m_bsInitialBelief.MaintainProblematicTag = true;
            Formula fReduced = Problem.Goal.Reduce(m_lObserved);
            if (fReduced.IsTrue(m_lObserved))
                m_bGoalState = true;
            else if (fReduced.IsFalse(m_lObserved))
                m_bGoalState = false;
            else
            {
        ...
        private State WriteTaggedDomainAndProblem(string sDomainFile, string sProblemFile, CompoundFormula cfGoal, List<Action> lActions, out int cTags, out MemoryStream msModels)
        {
            ...
            State sChosen = m_bsInitialBelief.WriteTaggedDomainAndProblem(sDomainFile, sProblemFile, cfGoal, lActions, out cTags, out msModels);
            foreach (GroundedPredicate gp in Observed)
                sChosen.AddPredicate(gp);
            return sChosen;
        }
        ...
        public static TimeSpan tsInUpdateClosed = new TimeSpan();
        ...
        private void AddRelevantVariables(Dictionary<GroundedPredicate, HashSet<GroundedPredicate>> dRelevant, Predicate pObservation)
        {
            if (m_dRelevantVariablesForPrecondition == null)
                m_dRelevantVariablesForPrecondition = new Dictionary<GroundedPredicate, HashSet<GroundedPredicate>>();
            foreach (GroundedPredicate gpPrecondition in dRelevant.Keys)
            {
                ...
                {
                    HashSet<GroundedPredicate> hs = new HashSet<GroundedPredicate>(dRelevant[gpPrecondition]);
                    hs.Add((GroundedPredicate)pObservation);
                    m_dRelevantVariablesForPrecondition[gpPrecondition] = hs;
                }
                else
                    m_dRelevantVariablesForPrecondition[gpPrecondition] = new HashSet<GroundedPredicate>(dRelevant[gpPrecondition]);
            }
        }

        public bool ConsistentWith(Dictionary<GroundedPredicate, HashSet<GroundedPredicate>> dRelevantForOther)
        {
            foreach (KeyValuePair<GroundedPredicate, HashSet<GroundedPredicate>> p in dRelevantForOther)
            {
        ...
        public static TimeSpan tsInIsClosed = new TimeSpan();
        public bool IsClosedState(Dictionary<string, List<PartiallySpecifiedState>> dClosedStates)
        {
            ...
            foreach (PartiallySpecifiedState pssClosed in dSamePosition)
            {
                ...
                if (ToString() == "(at p5-3)" && pssClosed.m_lOfflinePredicatesUnknown.Count == 0)
                    Console.Write("*");
