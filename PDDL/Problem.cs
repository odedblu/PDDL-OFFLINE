...
namespace PDDL
{
    public class Problem
    {
                                        ...
                                        List<Predicate> lTrueState, Dictionary<string, double> dFunctionValues, bool bOnlyIdentifyStates)
        {
            ...
            cfGoal = (CompoundFormula)cfGoal.Simplify();
            cfGoal.GetAllPredicates(lGoalPredicates);

            CompoundFormula cfKnowGoal = new CompoundFormula(cfGoal.Operator);
            ...
            foreach (Predicate p in lGoalPredicates)
            {
                ...
                else
                {
                    ...
                    cfKnowGoal.AddOperand(cfAnd);
                }
            }


            sw.WriteLine("(:goal " + cfKnowGoal.Simplify() + ")");
            //sw.WriteLine("))");
            if (SDRPlanner.Planner != SDRPlanner.Planners.FF)
            {
