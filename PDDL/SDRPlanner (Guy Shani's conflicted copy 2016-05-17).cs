        ...
        public void OfflinePlanning(string sPath, Domain domain, Problem problem)
        {
            ...
            while (stateStack.Count > 0)
            {
                ...
                if (bGoalReached)
                {
                    cGoalLeaves++;
                    /*
                    PartiallySpecifiedState pssTemp = pssCurrent;
                    while(pssTemp.Predecessor != null)
                    {                        
                        Console.WriteLine(pssTemp.GeneratingAction.Name + ", " + pssTemp.GeneratingObservation + ", " + pssTemp.ToString());
                        pssTemp = pssTemp.Predecessor;
                    }
                    */
                    pssCurrent.UpdateClosedStates(dClosedStates, domain, false);
