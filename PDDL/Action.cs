...
namespace PDDL
{
    public class Action
    {
        ...
        public Action NonConditionalObservationTranslation(Dictionary<string, List<Predicate>> dTags, List<string> lAlwaysKnown, bool bTrue)
        {
            ...
            //allow observations only for unknowns
            Predicate pKnowTrue = new KnowPredicate(pObserve);
            Predicate pKnowFalse = new KnowPredicate(pObserve.Negate());
            cfPreconditions.AddOperand(pKnowFalse.Negate());
            cfPreconditions.AddOperand(pKnowTrue.Negate());
            ...
            if (bTrue)
                aNew.Effects = new PredicateFormula(pKnowTrue);
            else
                aNew.Effects = new PredicateFormula(pKnowFalse);
        ...
        public Action AddTaggedConditions(Dictionary<string, List<Predicate>> dTags, List<string> lAlwaysKnown)
        {
            ...
            if (Effects != null)
            {
                ...
                foreach (Formula f in lObligatory)
                {
                    if (SDRPlanner.Planner != SDRPlanner.Planners.FF || !f.IsFunction())
                    {
                        f.GetAllPredicates(lKnowEffects);
                        cfEffects.AddOperand(f);
                    }
